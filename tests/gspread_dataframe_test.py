...
class TestWorksheetReads(unittest.TestCase):
    def setUp(self):
        ...
        self.spreadsheet = self.worksheet.exceptionally_access_underlying_spreadsheet()

    def test_noargs(self):
        df = get_as_dataframe(self.spreadsheet, self.worksheet)
    ...
    def test_evaluate_formulas_true(self):
        df = get_as_dataframe(self.spreadsheet, self.worksheet, evaluate_formulas=True)
        self.assertEqual(list(df.columns.values), COLUMN_NAMES)
    ...
    def test_usecols(self):
        df = get_as_dataframe(self.spreadsheet, self.worksheet, usecols=USECOLS_COLUMN_NAMES)
        self.assertEqual(list(df.columns.values), USECOLS_COLUMN_NAMES)

    def test_indexcol(self):
        df = get_as_dataframe(self.spreadsheet, self.worksheet, index_col=4)
    ...
    def test_indexcol_none(self):
        df = get_as_dataframe(self.spreadsheet, self.worksheet, index_col=False)
        self.assertEqual(len(df.columns), 10)
        self.assertEqual(df.index.name, None)
        self.assertEqual(type(df.index).__name__, "RangeIndex")
        self.assertEqual(list(df.index.values), list(range(9)))
    ...
    def test_header_false(self):
        df = get_as_dataframe(self.spreadsheet, self.worksheet, header=None)
        self.assertEqual(len(df), 10)

    def test_header_first_row(self):
        df = get_as_dataframe(self.spreadsheet, self.worksheet, header=0)
        self.assertEqual(len(df), 9)

    def test_skiprows(self):
        df = get_as_dataframe(self.spreadsheet, self.worksheet, skiprows=range(1, 4))
        self.assertEqual(len(df), 6)

    def test_prefix(self):
        df = get_as_dataframe(
            self.spreadsheet, self.worksheet, skiprows=[0], header=None, prefix="COL"
        )
        self.assertEqual(len(df), 9)
        self.assertEqual(
    ...
    def test_squeeze(self):
        df = get_as_dataframe(self.spreadsheet, self.worksheet, usecols=[0], squeeze=True)
    ...
    def test_no_nafilter(self):
        df = get_as_dataframe(self.spreadsheet, self.worksheet, na_filter=False)
        self.assertEqual(df["Dialect-specific implementations"][7], "")

    def test_nafilter(self):
        df = get_as_dataframe(self.spreadsheet, self.worksheet, na_filter=True)
        self.assertTrue(np.isnan(df["Dialect-specific implementations"][7]))

    def test_parse_dates_true(self):
        df = get_as_dataframe(self.spreadsheet, self.worksheet, index_col=4, parse_dates=True)
